...
<body class="my-body-content">
	<form id="columnEditForm" name="columnEditForm" class="my-form-content">
		...
		<table class="my-form-table-content">
			<tr>
				...
				<td class="my-form-table-td-content" style="width:200px;">
					<select id="column_name" name="column_name" class="form-control input-sm" style="width:180px;padding:5px;float:left;" readOnly></select>
				</td>
			</tr>
...
$(document).ready(function() {
	validator = $("#columnEditForm").validate({
		debug: true,
...
function initFlexFieldColumnInfo() {
	var url = "<%=context%>/app/sys/flexfield/getFlexFieldColumnInfo";
	url += "?t=" + new Date();
	...
	$.ajax({
		...
		success: function(data) {
			var json = JSON.parse(data);
			if (json.result_code == "Y") {
				var ffc = JSON.parse(json.result_info);
				dwr.util.setValues(ffc);
...
function doEditFlexFieldColumn() {
	var result = false;
	if (validator.form()) {
		...
		var url = "<%=context%>/app/sys/flexfield/editFlexFieldColumn";
		url += "?t=" + new Date();
