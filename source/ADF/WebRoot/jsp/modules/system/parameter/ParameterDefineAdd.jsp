...
<body class="my-body-content">
	<form id="parameterAddForm" name="parameterAddForm" class="my-form-content">
		<input type="hidden" id="parameter_type" name="parameter_type" value="<%=typeCode%>">
		...
		<table class="my-form-table-content">
			<tr>
				...
				<td class="my-form-table-td-content" style="width:200px;">
					<input id="parameter_code" name="parameter_code" type="text" class="form-control input-sm" style="width:180px;padding:5px;float:left;">
				</td>
			</tr>
	...
	validator = $("#parameterAddForm").validate({
		...
		rules: {
			parameter_code: {
				...
				isCode: true,
				remote: {
					url:"<%=context%>/app/sys/parameter/checkParameterCode?t=" + new Date(),
					type: "post",
					dataType: "json",
					data: {
						parameterCode: function(){return $('#parameter_code').val()}
					}
				}
			},
			parameter_name: {
...
function doAddParameterDefine() {
	...
	if (validator.form()) {
		...
		var url = "<%=context%>/app/sys/parameter/addParameterDefine";
		url += "?t=" + new Date();
