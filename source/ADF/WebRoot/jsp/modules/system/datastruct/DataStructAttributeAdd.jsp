...
<body class="my-body-content">
	<form id="structAttributeAddForm" name="structAttributeAddForm" class="my-form-content">
...
$(document).ready(function() {
	validator = $("#structAttributeAddForm").validate({
		debug: true,
		...
		rules: {
			attribute_code: {
				...
				isCode: true,
				remote: {
					url:"<%=context%>/app/sys/datastruct/checkDataStructAttributeCode?t=" + new Date(),
					type: "post",
					dataType: "json",
					data: {
...
function doAddDataStructAttribute() {
	var result = false;
	if (validator.form()) {
		var ddta = new SysDataStructAttribute();
		
		dwr.util.getValues(ddta);
		
		var json = JSON.stringify(ddta);
		
		var url = "<%=context%>/app/sys/datastruct/addDataStructAttribute";
		url += "?t=" + new Date();
