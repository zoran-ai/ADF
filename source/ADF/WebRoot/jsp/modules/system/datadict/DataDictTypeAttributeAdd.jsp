...
<head>
	...
	<script type='text/javascript' src='<%=context%>/js/json/model/SysDataDictTypeAttribute.js'></script>
...
<body class="my-body-content">
	<form id="typeAttributeAddForm" name="typeAttributeAddForm" class="my-form-content">
		...
		<table class="my-form-table-content">
			<tr>
				...
				<td class="my-form-table-td-content" style="width:200px;">
					<input id="attribute_code" name="attribute_code" type="text" class="form-control input-sm" style="width:180px;padding:5px;float:left;">
				</td>
			</tr>
...
$(document).ready(function() {
	validator = $("#typeAttributeAddForm").validate({
		debug: true,
		...
		rules: {
			attribute_code: {
				...
				isCode: true,
				remote: {
					url:"<%=context%>/app/sys/datadict/checkDataDictTypeAttributeCode?t=" + new Date(),
					type: "post",
					dataType: "json",
					data: {
						typeCode: function(){return $('#type_code').val()},
						attributeCode: function(){return $('#attribute_code').val()}
					}
				}
			},
			attribute_name: {
...
function doAddDataDictTypeAttribute() {
	var result = false;
	if (validator.form()) {
		var ddta = new SysDataDictTypeAttribute();
		
		dwr.util.getValues(ddta);
		
		var json = JSON.stringify(ddta);
		
		var url = "<%=context%>/app/sys/datadict/addDataDictTypeAttribute";
		url += "?t=" + new Date();
		
		var postData = "SysDataDictTypeAttribute=" + converSpecialCharOfUrl(encodeURI(json));
