...
<body class="my-body-content">
	<form id="typeAttributeEditForm" name="typeAttributeEditForm" class="my-form-content">
...
$(document).ready(function() {
	validator = $("#typeAttributeEditForm").validate({
		debug: true,
...
// 初始化数据字典类型属性信息
function initDataDictTypeAttributeInfo() {
	var url = "<%=context%>/app/sys/datadict/getDataDictTypeAttributeInfo";
	url += "?t=" + new Date();
	...
	$.ajax({
		...
		success: function(data) {
			var json = JSON.parse(data);
			if (json.result_code == "Y") {
				var ddta = JSON.parse(json.result_info);
				dwr.util.setValues(ddta);
...
function doEditDataDictTypeAttribute() {
	var result = false;
	if (validator.form()) {
		...
		var url = "<%=context%>/app/sys/datadict/editDataDictTypeAttribute";
		url += "?t=" + new Date();
